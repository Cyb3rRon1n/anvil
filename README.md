...
**Status:** Guided TUI by default, plus a scriptable CLI (`--plain`/`--non-interactive`) — detects your GPU's real VRAM (a functional query, not just "is a tool installed"), recommends a tier, generates a Docker Compose stack for Ollama + Open WebUI (+ ComfyUI and/or InvokeAI at Heavy tier — ComfyUI has a real, verified image for NVIDIA, AMD, and Intel Arc; InvokeAI for NVIDIA and AMD). Verified against real containers on real hardware where possible; the honest gap (neither image-generation option has run against real discrete GPU compute — none exists in this project's environment, though InvokeAI has been started and confirmed serving on CPU) is documented in [CLAUDE.md](CLAUDE.md), not hidden.
